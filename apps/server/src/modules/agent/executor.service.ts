// ExecutorAgent：执行计划中的单个子任务 + 重试。逐行迁自 enterprise/agent/executor.py。
//
...
/// action_handler 的返回形状。源码 executor.py:30-31。
...
/// 注入的动作处理器。源码 action_handler: async (goal, context) -> dict。
...
export class ExecutorService {
  ...
  /// 带重试地执行一个子任务。源码 execute_subtask。
  async executeSubtask(
    ...
    // 单调时钟计时（不受系统时钟回拨影响）。对齐源码 time.monotonic()。
  ...
  /// 无 action_handler 时的模拟执行（测试/开发用）。源码 _simulate_execution。
